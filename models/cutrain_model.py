...
class CUTRAINModel(BaseModel):
    ...
    def forward(self):
        ...
        self.image_transferred = self.netG(self.image_to_transfer)
    ...
    def compute_G_loss(self):
        ...
        self.loss_DisNCE = self.calculate_DisNCE_loss(self.pred_B, self.O) * self.opt.lambda_DisNCE

        self.loss_G = self.loss_G_GAN + loss_NCE_both + self.loss_DisNCE
        return self.loss_G
    ...
    # disentangle NCE
    def calculate_DisNCE_loss(self, pred_B, O):
        pred_R = O - pred_B
        batchsize = self.opt.num_patches
        feat_B, sample_ids = self.netF(pred_B, batchsize*2, None)
        feat_R, _ = self.netF(pred_R, batchsize*2, None)

        criterNCE = PatchNCELoss(opt).to(self.device)
        
        # background
        feat_B1 = torch.cat((feat_B[0,:], feat_R[0:batchsize,:]), dim=0)
        feat_B2 = torch.cat((feat_B[1,:], feat_R[batchsize:,:]), dim=0)
        loss_B_DisNCE = criterNCE(feat_B1,feat_B2)

        # rain
        feat_R1 = torch.cat((feat_R[0,:], feat_B[0:batchsize,:]), dim=0)
        feat_R2 = torch.cat((feat_R[1,:], feat_B[batchsize:,:]), dim=0)
        loss_R_DisNCE = criterNCE(feat_R1,feat_R2)

        #total loss
        return (loss_B_DisNCE+loss_R_DisNCE)*0.5